...
class PointControllerIntegrationTest : DescribeSpec() {
    ...
    init {
        describe("GET /point/{id}") {
            ...
            it("존재하지 않는 유저의 포인트를 조회하면 0 포인트를 반환한다") {
                // given
                val userId = 999L
        ...
        describe("PATCH /point/{id}/charge") {
            ...
            it("0 이하의 금액으로 충전하면 400 에러가 발생한다") {
                // given
                val userId = 5L
        ...
        describe("PATCH /point/{id}/use") {
            ...
            it("100 단위가 아닌 금액으로 사용하면 400 에러가 발생한다") {
                // given
                val userId = 8L
                userPointTable.insertOrUpdate(userId, 1000L)
            ...
            it("1000을 초과하는 금액으로 사용하면 400 에러가 발생한다") {
                // given
                val userId = 9L
        ...
        describe("동시성 테스트") {
            it("동일 유저에 대한 동시 충전 요청이 올바르게 처리된다") {
                // given
                val userId = 10L
                ...
                successCount.get() shouldBe numberOfThreads
                finalPoint.point shouldBe chargeAmount * numberOfThreads
            }

            it("동일 유저에 대한 동시 사용 요청이 올바르게 처리된다") {
                // given
                val userId = 11L
                ...
                val finalPoint = userPointTable.selectById(userId)
                successCount.get() shouldBe numberOfThreads
                finalPoint.point shouldBe initialPoint - (useAmount * numberOfThreads)
            }
        }

        describe("통합 시나리오") {
            it("충전, 사용, 조회가 연속적으로 올바르게 동작한다") {
                // given
                val userId = 12L
